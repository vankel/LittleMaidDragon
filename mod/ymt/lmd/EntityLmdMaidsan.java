...
public class EntityLmdMaidsan extends LMM_EntityLittleMaid {
	...
	public boolean interact(EntityPlayer player) {
		...
		if (isCommandWaiting(player)) {
			if (isMaidHanded(Item.saddle) && Utils.tryUseItems(player, Item.sugar, true)) {
				eatSugar(false, true);
				if (switchDragon(player, dragonData)) {
	...
	public void onLivingUpdate() {
		...
		if (core.isEnableFallingCatcher()) {
			...
			if (player != null && core.isFalling(player)) {
				if (switchDragon(player, dragonData)) {
					setDead();
					return;
				}
	...
	public boolean switchDragon(EntityPlayer player, NBTTagCompound tag) {
		...
		dragon.setLifeStageForce(LifeStage.ADULT);
		dragon.setEntityMaid(this); // ドラゴンをロードした後に改めて設定
		dragon.setEntityHealth(MathHelper.ceiling_double_int(this.getHealth() * (double) dragon.getMaxHealth() / this.getMaxHealth()));
		dragon.setPathToEntity(null);
		dragon.setAttackTarget(null);
		dragon.setLocationAndAngles(posX, posY, posZ, MathHelper.wrapAngleTo180_float(player.worldObj.rand.nextFloat() * 360.0F), 0);
